document.addEventListener('DOMContentLoaded', () => {
    ...
    function renderStartScreen(data) {
        ...
        document.getElementById('subject-select').addEventListener('change', (event) => {
            ...
            if (selectedSubject) {
                ...
                answers = Array(selectedQuiz.length).fill(null); // Reset answers
                renderQuizScreen();
            }
        });
    }
    ...
    function renderQuizScreen() {
        if (currentQuestionIndex >= selectedQuiz.length) {
            renderScoreScreen(); // If all questions are answered, go to score screen
            return;
        }
        ...
        document.querySelectorAll('.option-button').forEach(button => {
            button.addEventListener('click', () => {
                ...
                answers[currentQuestionIndex] = selectedOption; // Save user's answer
                renderQuizScreen();
            });
        });
    ...
    function renderScoreScreen() {
        ...
        document.getElementById('retry-button').addEventListener('click', () => {
            renderStartScreen(questionsData); // Reset to start screen
        });
